...
<style>
	...
	.portfolio_slide:not(.is-active) img { -ms-transform: scale(.9);transform: scale(.9); }
	.portfolio_slide img { border-radius: 8px; transition: transform .4s cubic-bezier(.19,1,.22,1); -ms-transform-origin: center;transform-origin: center; }
				...
				{foreach $folio as $item}
					<li class="glide__slide portfolio_slide cursor-grab" data-index="{$item@index}">
						<div class="flex flex-col">
							<div class=" flex-shrink md:w-1/3 mx-auto text-center">
							...
							<div class="flex-grow mr-10 ">
								<img class="screenshot img-fluid" src="{asset path=$item['screenshot']}" alt="{$item['title']} Screenshot">
							</div>
						</div>
					</li>
	            {/foreach}
...
{js}
	<script>
		(function() {
			...
			const CustomActiveClass = function (Glide, Components, Events) {
				var Component = {
					...
					changeActiveSlide() {
						...
						$('.portfolio_slide').removeClass('is-active');
						$('.portfolio_slide').removeClass('is-next');
						$('.portfolio_slide').removeClass('is-prev');
			...
			document.querySelector('#{$id} .glide__slides').addEventListener('click', function(e) {
