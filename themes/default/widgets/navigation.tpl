...
<header style="{$style|default}">
    ...
    {/css}
				...
				<li class="{$liClass}" >
                    ...
                    {if isset($link.children)}
						<div  style="transition-timing-function: cubic-bezier(.2, 1, .22, 1)"
	                          class="{$key}_dropdown transform -translate-x-1/2 left-1/2 pointer-top absolute z-50 shadow-3xl bg-white md:p-8 lg:p-9 rounded-md origin-top transition duration-300 pointer-events-none group-hover:pointer-events-auto opacity-0 group-hover:opacity-100 transform scale-95 group-hover:scale-100">
							<ul class="block m-0 md:w-80 lg:w-96 grid md:gap-6 lg:gap-9">
	                            {foreach $link.children as $child}
		                            {include file="nav_menu_item.tpl" link=$child.link title=$child.title sub=$child.sub|default:'' icon=$child.icon}
	                            {/foreach}
							</ul>
						</div>
                    {/if}
		...
		<!-- Mobile menu -->
		<nav x-data="{ isOpen: false }" class="fixed md:hidden inset-x-0 top-0 z-40">
			...
			<ul :class="isOpen?'block':'hidden'" class="md:hidden">
                {if isset($link.children)}
                    ...
                    {/foreach}
				{else}
                    {include file="nav_menu_item.tpl" link=$link.link title=$link.title sub=$link.sub|default:'' icon=$link.icon}
                {/if}
			</ul>
            {/foreach}
...
{*{navigation_original}*}
{*
In order to get nice smooth animations that don;t do strange things we have to use vue
