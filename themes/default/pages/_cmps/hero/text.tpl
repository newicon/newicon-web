<div class="pt-10 pb-0 text-center md:text-left {$class|default:''}">
	<div class="px-0 {(!empty($bgTitle)) ? 'ni-bg-title' : ''}" data-title="{$bgTitle|default:''}">
		...
		<h4 class="ni-italic {(empty($body))?'mb-0':'mb-10'} text-xl md:text-2xl md:font-normal {$subClass|default:'text-gray-400'}">{$sub}</h4>
        {if isset($body)}<p class="text-lg md:text-xl md:font-normal {$bodyClass|default:'text-gray-600'}">{$body}</p>{/if}
